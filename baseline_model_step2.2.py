...
def calculate_metrics(y_true, y_pred, inverse_transform=False):
    """计算评估指标"""
    if inverse_transform:
        # 如果是 log 变换后的预测，需要转换回原始价格计算 RMSE/MAE
        y_true_orig = np.expm1(y_true)
        y_pred_orig = np.expm1(y_pred)
        return {
            'RMSE': np.sqrt(mean_squared_error(y_true_orig, y_pred_orig)),
            'MAE': mean_absolute_error(y_true_orig, y_pred_orig),
            'R2': r2_score(y_true, y_pred)  # R2 使用变换后的值计算
        }
    return {
...
def main():
    ...
    # 对目标变量进行 log1p 变换，解决价格数据的右偏分布问题
    ...
    # 使用 GradientBoosting 模型 (改进参数)
    model = GradientBoostingRegressor(n_estimators=200, max_depth=6, min_samples_split=5, 
    ...
    # 计算指标 (使用 inverse_transform=True 来正确计算 RMSE/MAE)
    metrics = calculate_metrics(y_test, y_pred, inverse_transform=True)
    print_metrics(metrics, "\n完整特征模型性能 (使用 log1p 变换):")
